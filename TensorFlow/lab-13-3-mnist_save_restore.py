...
training_epochs = 15
batch_size = 100
CHECK_POINT_DIR = TB_SUMMARY_DIR = './tb/mnist2'  # 요약 디렉토리
...
# 가중치와 바이어스 weights & bias for nn layers
...
# 요약 기록기 생성 Create summary writer
writer = tf.summary.FileWriter(TB_SUMMARY_DIR)
writer.add_graph(sess.graph)
global_step = 0

# 저장 및 복원 Saver and Restore
...
# 검증 및 정확도 Test model and check accuracy
correct_prediction = tf.equal(tf.argmax(hypothesis, 1), tf.argmax(Y, 1))
accuracy = tf.reduce_mean(tf.cast(correct_prediction, tf.float32))
print('Accuracy:', sess.run(accuracy, feed_dict={X: mnist.test.images, Y: mnist.test.labels, keep_prob: 1}))

# 랜덤 예측 Get one and predict
