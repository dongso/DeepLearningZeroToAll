...
class Model:  # Model 클래스
    ...
    def predict(self, x_test, keep_prop=1.0):  # 예측
        return self.sess.run(self.logits, feed_dict={self.X: x_test, self.keep_prob: keep_prop})

    def get_accuracy(self, x_test, y_test, keep_prop=1.0):  # 정확도
        return self.sess.run(self.accuracy, feed_dict={self.X: x_test, self.Y: y_test, self.keep_prob: keep_prop})

    def train(self, x_data, y_data, keep_prop=0.7):  # 학습
...
for epoch in range(training_epochs):  # 에폭 반복
    ...
    print('Epoch:', '%04d' % (epoch + 1), 'cost =', '{:.9f}'.format(avg_cost))  # 에폭, 평균 비용
