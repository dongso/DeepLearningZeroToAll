# Lab 12 문자 시퀀스 RNN. Character Sequence RNN
...
dic_size = len(char2idx)  # RNN 입력 크기(원핫 크기). RNN input size (one hot size)
hidden_size = len(char2idx)  # RNN 출력 크기. RNN output size
num_classes = len(char2idx)  # 최종 출력 크기(RNN이나 소프트맥스 등) final output size (RNN or softmax, etc.)
batch_size = 1  # 샘플 데이터(배치 크기) one sample data, one batch
...
# 시퀀스 손실 계산을 위한 reshape. reshape out for sequence_loss
...
with tf.Session() as sess:
    ...
    for i in range(50):
        ...
        result_str = [idx2char[c] for c in np.squeeze(result)]  # print char using dic
