# Lab 10 MNIST와 딥러닝 CNN MNIST and Deep learning CNN
...
# 드롭아웃 비율 플레이스홀더. dropout (keep_prob) rate  0.7~0.5 on training, but should be 1 for testing
keep_prob = tf.placeholder(tf.float32)
# 입력 플레이스홀더 input place holders
X = tf.placeholder(tf.float32, [None, 784])
X_img = tf.reshape(X, [-1, 28, 28, 1])   # 이미지 img 28x28x1 (black/white)
...
def evaluate(X_sample, y_sample, batch_size=512):  # 정확도 계산 함수
...
print("\nAccuracy Evaluates")
print("-------------------------------")
print('Train Accuracy:', evaluate(mnist.train.images, mnist.train.labels))  # mnist train 이미지 정확도
print('Test Accuracy:', evaluate(mnist.test.images, mnist.test.labels))  # mnist test 이미지 정확도

# 랜덤 데이터 예측 Get one and predict
print("\nGet one and predict")
print("-------------------------------")
